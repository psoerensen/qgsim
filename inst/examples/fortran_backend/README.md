# Optional Fortran shared-library backend example

This developer example shows the shape of a qgsim backend that calls a native
Fortran subroutine through `.Fortran()`. The native subroutine only adds a
generation-specific increment to a numeric vector. It is intentionally a toy
calculation, not a scientifically complete breeding-scheme simulator.

The R script keeps backend responsibilities separate:

- `fortran_translate()` converts a validated `qgsim_plan` to simple native input.
- `fortran_execute()` calls the shared-library subroutine for each generation.
- `fortran_collect()` converts native output to a validated `qgsim_result`.
...
Building requires the Fortran toolchain used by `R CMD SHLIB`. The example is
